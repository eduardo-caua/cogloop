...
### 1. Pick ticket
- Query the GitHub Project for the next ticket in the **Todo** column (use `gh project item-list`)
...
Run all four speckit commands sequentially in the `speckit.workspaceDir`. Pass the ticket title and body as input to `speckit.specify`.

**specify** → Read the ticket title and body from GitHub Issue. Run `/speckit.specify` with the full ticket description as input.
...
In the spec repo (`speckit.specsDir`):
- The branch was created by speckit during specify (e.g. `012-feature-name`)
- Push the branch: `git push origin <branch>`
- Create a PR: `gh pr create --title "<ticket title>" --body "Spec for <ticket title>"`
- Add the PR link as a comment on the GitHub Issue
